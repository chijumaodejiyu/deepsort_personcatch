...
class CH341TI2C:
    ...
    def set_i2c_speed(self, speed=3):
        """
        设置iic口流模式（用于指定速度）
    ...
    def write(self, i2c_addr, reg_addr, dat):
        ...
        #result = self.write_stream(i2c_addr, reg_addr, dat)
        return result

    def read_stream(self, i2c_addr, reg_addr):
        if self.dll.CH341OpenDevice(self.usb_id) != -1:
            obuf = (c_byte * 2)()
            ibuf = (c_byte * 1)()
            obuf[0] = i2c_addr
            obuf[1] = reg_addr
            self.dll.CH341StreamI2C(self.usb_id, 2, obuf, 1, ibuf)
    ...
    def write_stream(self,  i2c_addr, reg_addr, dat):
        if self.dll.CH341OpenDevice(self.usb_id) != -1:
            obuf = (c_byte * 3)()
            ibuf = (c_byte * 1)()
            obuf[0] = i2c_addr
            obuf[1] = reg_addr
            obuf[2] = dat & 0xff
            result = self.dll.CH341StreamI2C(self.usb_id, 3, obuf, 0, ibuf)
            self.dll.CH341CloseDevice(self.usb_id)
            return result
        else:
            print("USB CH341 Open Failed!")
...
if __name__ == '__main__':
    i2c = CH341TI2C(dll_path="CH341DLLA64.dll")
    while True:
        print(i2c.write_stream(0x40, 0x00, 0x00))
        print(i2c.read_stream(0x40, 0x00))
        time.sleep(0.0005)